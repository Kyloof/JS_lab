...
def load_timeseries(csv_path, loaded_data):
        ...
        for i in range(len(station_codes)):
            result.append(TimeSeries(measure_names[i], station_codes[i], avg_times[i], units[i], timestamps, list(map(safe_float,measurements[i]))))

        loaded_data.extend(result)
        return result
