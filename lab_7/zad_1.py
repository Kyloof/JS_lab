...
def median(xs: list[int]):
    def get_len(xs):
        return 0 if xs == [] else 1 + get_len(xs[1:])
    def get_median(xs, idx, length):
        head, *tail = xs
        snd_head, *snd_tail = tail
        return ((head+snd_head)/2 if idx == (length-1)//2 else get_median(tail, idx + 1, length)) if length%2 == 0 else (head if idx == length//2 else get_median(tail, idx + 1, length))
    length = get_len(xs)
    return get_median(xs, 0, length) if length > 1 else xs[0]
